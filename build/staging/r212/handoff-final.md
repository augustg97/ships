...
**The close ratchet, RUN WHOLE in one attempt (build/staging/r212/close-ratchet.out, 14:02:38
→ 14:42:16, ~37 s a frame under load 4–9): 64 frames, all within tolerance, exit 0, zero
movers — exactly r212/PREDICTIONS-close.md, written at 14:04 before the run finished. The cog's
berth neighbours ship-dhow 0.000% / 0.000, ship-treasure 0.000% / 0.001, aboard-treasure
0.002% / 0.000; shipwright 0.000%; globe-default at its documented 0.046% / 0.011 flap a
twelfth round. The cog has no frame of her own (r173), so the Spill's rendered check is the
three witness frames above. No readiness abort. Rule 0 answered on r212/spill-astern.png, read
whole: it reads as a rendered world — a single square sail bellying over a laid deck with its
gratings, the open water beyond, the castle's plank tones and its stanchion parapet in
perspective. Three facts a viewer can read off it without a legend: an eight-sided oak capstan
head on a cone stands on a square step in the castle deck, two handspikes shipped through it
at right angles; the castle deck is a raised open platform railed round, standing over the
afterdeck; the yard's halyard leads down the mast to the deck forward of the castle.**
...
**Build PUBLISHED, data-version 1788471763, first paint 8.54 MB of 8.6, docs/ 155 files;
web/index.html carries the same stamp and is in this commit's path list (the r209 rule). The
