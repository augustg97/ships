...
$(function(){
	...
	$.funcFaq = function (){
		...
		this.opt = {
				...
				,"searchCnd":""
				,"searchWrd":""
				,"searchCategory":"ACARPT001"
				,"url":"/resources/academicreport"
				,"nttId":""
		...
		this.listInit = function(curPage) {
			...
			if($('#search_category').val() !=null && $('#search_category').val() !=''){
				_this.opt.searchCategory = $('#search_category').val();
				
				$('.tab-wrap >ul >li').each(function(index, item){ 
					var catId = $('.tab-wrap >ul >li').eq(index).find('a').data('id');
					if(_this.opt.searchCategory == catId){
						$('.tab-wrap >ul >li').eq(index).addClass('active');
					}
				});
			}else{
				$('.tab-wrap >ul >li').eq(0).addClass('active');
			}
		...
		this.infoInit = function(nttId) {
			...
			_this.opt.searchCategory = $('#searchCategory').val();
			_this.getInfo();
		}
