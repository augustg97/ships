import json, sys
from playwright.sync_api import sync_playwright

OUT = sys.argv[1] if len(sys.argv) > 1 else "build/staging/survey-r144.json"
with sync_playwright() as pw:
    browser = pw.chromium.launch()
    try:
        page = browser.new_page(viewport={"width": 1440, "height": 900})
        page.goto("http://localhost:8149/?frozen=1#v=ship", wait_until="load", timeout=60000)
        page.wait_for_function("window.__FRAME_READY === true", timeout=60000)
        out = page.evaluate(
            "async () => { const s = await (await fetch('/survey-hulls.js')).text();"
            " return (0, eval)(s); }")
    finally:
        browser.close()
json.dump(out, open(OUT, "w"), indent=1)
for h in out["crudestFirst"]:
    if h["id"] in ("panokseon", "sekibune", "galleass", "galley", "trireme"):
        print(h["id"], "meshes", h.get("meshes"), "boxes", h.get("boxMeshes"),
              "boxPct", h.get("boxPct"), "tris", h.get("tris"))
print("hulls:", len(out["crudestFirst"]), file=sys.stderr)
