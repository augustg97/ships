...
function buildFittings(S, group, mats) {
  ...
  /* ── THE KOREAN WOODEN ANCHOR AND ITS STONE, FROM THE RECORD ────────────────────────
     ...
     wooden anchors beside them radiocarbon-dated 2nd c. BC through Joseon. The frame
     and the stone's station are the institute's own reconstruction (r194): the 2023
     report 『한국의 닻돌』 names lashing to the shank's OUTSIDE the leading use for
     the plank-form stones, the 19th-c. 표민대화 fixes the stone 닻채 중앙부 — at the
     shank's MIDDLE — and the institute's figure draws it there (centre read 0.57 of
     the shank above the foot; drawn 0.55, the shared constant V-WSTATION audits).
     The arms are timbers CROSSING the shank, whipped where they cross, blunt upper
     ends standing to the stone's underside to brace it, carved points splayed below,
     a spreader board across the splay; the stone rides the shank's face with end
     turns in its own grooves and crossed frapping over the midbody (the 2021
     reproduction's lashing). No stone has been found still lashed on — the report
     says so, and the provenance carries it. The drawn ship is under way, so the
  ...
  if (S.woodAnchor) {
    ...
    /* the crossed-arm frame: each 닻가지 is a timber crossing the shank — carved
       ...
       whipped where it crosses. Four hooks are two pairs in perpendicular planes; a
       two-hook record draws one pair. The hook limb (crossing to point) is the
       record's armM; the blunt limb is the figure's 0.70 of it. */
    const TH = 0.78;                                    /* splay from the shank, rad */
    const yX = -shankL * 0.70;                          /* mean crossing station */
    const BL = armL * 0.70, HL = armL;
    /* the figure whips the two pairs at STAGGERED stations, ~0.12 of the shank
