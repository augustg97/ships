#!/usr/bin/env python3
"""r177, read-only: the drawn shells' half-breadth and deck height at bow stations,
for the corbita, BEFORE any record is written. The windlass
...
async () => {
  ...
  for (const id of ['corbita']) {
    ...
    if (plank) {
      ...
      for (const u of [0.06, 0.08, 0.10, 0.12, 0.14, 0.16, 0.18, 0.20, 0.22]) {
        const sx = (u - 0.5) * L;
        let zmax = 0, ytop = -1e9;
