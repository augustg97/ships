...
function buildFittings(S, group, mats) {
  ...
  if (S.yotsumeAnchor) {
    ...
    /* local frame: origin at the crown, shank along +Y; the teardrop head ring
       ...
       MEMBER SECTIONS ARE THE RECORD'S (r191): 表1 of the corpus paper (Matsui
       2013 p.38) measures all 49 anchors at member level — the arms are
       RECTANGULAR forged bars, 最大幅×最大厚 at the root falling to a wide thin
       blade at the fluke tip. Corpus means for the ≥190 cm class (fractions of
       全長): root 0.0346×0.0198, tip 0.0220×0.0058. Both rings take the type
       anchor the class already cites (Onominato a, #29: AR 44×19 bar 3 on 269;
       AcR ⌀36 section 4×4.5). The SHANK is the one member the table does not
       record: its square side is SOLVED so the drawn iron weighs the record's
       kg at 7850 kg/m³ — the mass is the corpus's one weighed anchor
       (Kozushima, 2.8 m, 330–340 kg by forklift, 約90貫) cube-scaled through
       the record field. The paper's own monument photographs read slimmer
       shanks (0.023–0.034 of 全長, obliquity unbounded); the weighed record
       outranks that read, and the disagreement is named in the provenance. */
    const YA_P = {
      ...
      shTaper: 0.72                  /* drawn class default — unrecorded */
    };
    ...
    const shS = Math.sqrt(Math.max(1e-9, (yaKg / 7850 - vOther)
      / (shankL * (1 + tp + tp * tp) / 3)));       /* square side at the crown */
    const ai = new THREE.Group();
    ai.name = 'ya-grp';
