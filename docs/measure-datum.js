/* ── FLOAT-DATUM MEASUREMENT, round 33 ──────────────────────────────────────────────────
 * The Shipwright floats every ship at waterY = keelBottom + draught, where keelBottom is the
 * Box3 min of the WHOLE built group. surfacePoint() puts the design waterline at local y = 0
 * by construction (v = 0.62 → z = 0), so if anything hangs below the hull skin — keel timber,
 * rudder heel, screw, bulb — the water lands below the marks by exactly that overhang, and the
 * ship shows antifouling above the still-water line. This measures the overhang per ship and
 * names the deepest part. Run in-page: fetch + eval, like the audit.
 */
(function measureDatum() {
  ...
  for (const v of list) {
    ...
    rows.push({
      ...
      /* how far the current formula puts the water BELOW the design waterline (negative =
         that many metres of bottom paint in the air) */
