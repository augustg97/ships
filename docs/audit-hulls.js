...
const bbx = new THREE.Box3().setFromObject(o);
booms.push({ cx: (bbx.min.x + bbx.max.x) / 2, len: Math.hypot(bbx.max.x - bbx.min.x, bbx.max.y - bbx.min.y) });
} });
if (booms.length >= 2) {
...
g.traverse(o => { const p = tagOf(o); if (!o.isMesh || !p) return;
if ((p.key === 'pinRail' && p.name === 'Belaying pins') || (p.key === 'fifeRail' && p.name === 'Fife rail pins')) pinsM.push(o);
else if (p.key === 'sheet' || p.key === 'tack' || p.key === 'halyard' || p.key === 'brace'
|| p.key === 'throatHalyard' || p.key === 'peakHalyard' || p.key === 'jeers') ropeM.push(o);
...
else if (p.key === 'shroudLashing' && o.userData.falls) falls.push({ pts: worldS(o), ...o.userData.falls });
...
if (Math.min(da, db) > len) continue;
...
if (o.userData.segs && o.userData.mast !== undefined && o.userData.feetKind !== undefined) {
