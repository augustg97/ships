/* Injection proof for 'open boom off its sail plan', long side: stretch every
   aftermost gaff/spanker boom to 130% along its length (cylinder length is LOCAL Y;
...
(() => {
  ...
  SHIPS_HULL.buildShip = function (S, opts) {
    ...
    if (mk && (mk.rig === 'gaff' || (mk.rig === 'square' && mk.spanker))) {
      ...
      if (aft) { aft.o.scale.y = 1.3; g.updateMatrixWorld(true); }
    }
    return g;
  };
})()
