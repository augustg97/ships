...
def main():
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            # AUDIT_URL picks the origin: :8150 for a staged copy, a docs/ server for the
            # published record (r274: the audit must read the split provenance file there)
            base = os.environ.get("AUDIT_URL", "http://localhost:8149").rstrip("/")
            page.goto(base + "/?frozen=1#v=ship", wait_until="load", timeout=60000)
