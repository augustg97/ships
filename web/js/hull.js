...
/* ── THE SECTION FORM CHANGES ALONG THE SHIP (round 227) ─────────────────────────────
   ...
   (hull.section.stations[] { spant | u, floorHalfFrac, power }); the loft interpolates F
   and n linearly in u between them and holds the end values beyond the outermost. A
   station named by frame number is placed by frameU. Without stations, the record's
   single form everywhere, byte-identical to r226. */
...
function buildFramesGeometry(S, NF = 26, onlyU) {
  ...
  let base = 0;
  for (const u of us) {
    /* ── A FRAME IS A PLANE AT A FIXED x (round 240) ──────────────────────────────────
       A frame is a timber set square across the keel: every point of it stands at one x.
       The builder drew each frame at a constant u instead, and surfacePoint's x at a
       station is not constant over its height wherever the end leans — the class rake above
       the water on every raked hull, the whole height under a straight stem or post (r237,
       r239) — so a bow frame leaned with the stem: Frame 1 of 30 on the cog spanned 2.90 m
       of x from its floor to its head (r239/measure-after.out), a timber drawn on a slant no
       shipwright ever cut. The frame's x is the skin's at its station AT THE WATERLINE
       (v 0.62, where z is 0 and every rake term vanishes — the x frameStations placed it by),
       and at every other height the builder finds the u whose skin stands at that x: a
       bisection over u, x(u) at a fixed height being monotone (the stem's offset is under
       lwl·forefoot/2, r239). Where the skin's x does not vary with height the inversion
       returns the station's own u untouched, so an unraked hull, and every height under the
       water on a class hull, is byte-identical. Where the plane misses the skin — forward of
       the stem's line below the water on the cog, whose stem stands 3.25 m abaft the
       waterline's end at the keel — the frame has no skin to sit against, and it ENDS there:
       its foot lands on the stem (or the post) and is cut square in the open, as the
       floors nearest a raked stem land on the stem and the apron rather than the keel. */
    ...
    for (let sgn = -1; sgn <= 1; sgn += 2) {        // both sides of the ship
      if (!REC) {
        ...
        for (let j = 0; j <= NV; j++) {
          const v = j / NV;
          const p = P(v); if (!p) continue;         // the plane misses the skin at this height
      ...
      /* the head stands a hand under the rail AT THE FRAME'S x (round 240): the sheer is read
         at the u whose skin stands at xF at the head's height, not at the station's nominal u */
      ...
      /* ── THE FUTTOCK'S HEAD IS A HEWN END, NOT A SAWN CAP (round 223) ─────────────────
         The Bremen cog's futtocks (r213/wreck-dsm-side.jpg, the port side forward, ~45 px
         to a siding) narrow toward their heads to about three quarters of the siding they
         carry at the deck, and the head itself is rounded off, a grown timber's end dressed
         with the axe. The record says how much (frames.headSidedFrac), over what height
         (frames.headTaperM, measured down from the head) and whether the head is rounded
         (frames.headRound); without those fields the timber keeps its one siding and its
         square cap, byte-identical. The round is a semicircle in the siding plane over the
         head's own half-siding, so the extra stations it needs are put in by HEIGHT under
      ...
      if (ty.length < 2) continue;                    // the plane misses the skin entirely
      const NT = ty.length - 1, yTop = ty[NT], yKeel = ty[0], vLow = tv[0];
      const cut = vLow > 1e-9;                        // the foot lands on the stem or the post, not the keel
      const vAtY = y => {
      ...
      const yKeelTrue = surfacePoint(S, H, u, 0)[1];   // the keel under the station, cut or not
      ...
      for (const T of timbers) {
        if (T.yHead <= yKeel + 0.02) continue;         // a timber wholly below the cut is not built
        ...
        for (let j = 0; j <= NS; j++) {
          const v = vs[j];
          /* the inward normal is the SECTION's, at this point's own u: a difference taken along the
             frame's plane is not a section tangent where the plane runs out through the stem (the
             first station at the cut read 0.05 m higher than the second and the audit split it off
             as a timber of its own) */
        ...
        if (T.yFoot > yKeel + 1e-6 || cut)              // an upper timber's foot, or a foot on the stem: cut square in the open
...
function castleGeom(S, H) {
  ...
  /* the step (after part to wing) and the wing's forward end: the plate's own stations
     ...
     the plan's lengths chained aft to fore from the after edge */
...
function surfacePoint(S, H, u, v) {
  ...
  const sec = flared ? sectionAt(S, H.section, u) : null;   // F and n at THIS station (round 227)
  const flaredY = (h, v) => {
  ...
  S.masts.forEach((mk, mi) => {
    ...
    const crossYard = (yy, yardLen, kind, hoist) => {
      ...
      /* r258: what holds the yard there — the iron hull's fixed lower yard hangs in chain
         slings on an iron truss; every hoisting yard, and every yard of the wooden rig, is
         held to the mast by a rope parrel (Falconer 1780) */
      ...
      /* ── THE FITTING THAT HOLDS THE YARD THERE (round 258, 0y²⁷) ─────────────────────
         ...
         — the lateen (r254) and the lug (r253) carry their parrels, and the square yard
         stood at the truss's distance on air. A hoisting yard, and every yard of the wooden
         rig, is held by a PARREL: one rope loop at the slings, round the mast's AFTER side
         at the mast's radius there, made fast to the yard's after face either side of the
         pole, in the plane square to the raked axis — the lateen's loop turned to the fore
         face. On the iron hulls the fixed lower yard hangs in chain slings and is not
         hoisted; an iron TRUSS holds it — a band round the mast, a band round the yard's
         middle and a goose-neck between them. Which fitting is the record's rig (S.iron and
         the yard's hoist); the throw stays the two radii, the yard against the pole, and the
         yard's record names the fitting (userData.slung.held). Tagged 'parrel' / 'truss' so
         the audit can read each yard's fitting off the meshes (D-YARD-HELD). */
      {
        const C = new THREE.Vector3(mxA(yy), yy, 0);            // the axis at the slings
        const N = new THREE.Vector3(nX, nY, 0);                  // the axis's forward normal
        ...
        if (HELD === 'truss') {
          ...
          const arc = [];
          for (let a = 50; a <= 310; a += 20) {                  // a = 180 is dead aft
            const ca = Math.cos(a * Math.PI / 180), sa = Math.sin(a * Math.PI / 180);
            arc.push(C.clone().addScaledVector(N, Rp * ca).setZ(Rp * sa));
          }
          /* the two ends: on the yard's after face, either side of the mast, where the
             braced spar actually is (its arms swing fore and aft with the trim) */
    ...
    segs.forEach((seg, si) => {
      ...
      if (FINE && mk.rig === 'square' && si === 0 && (S.year || 0) >= 1100) {
        ...
        /* r235: THE RECORD MAY ATTEST THE TOP'S FORM. The class platform (round and walled
           before 1710, D-shaped after) is a great ship's fighting top, sized off the beam;
           the cog's masthead on the Elbing seal of 1350 is a battlemented BASKET on the
           pole — a staved bucket flaring to its rim, the mast through its floor — and the
           Ubena von Bremen carries one 1.8 m across. `top.form 'basket'` on the mast record
           draws that, at the record's rim, base, height and pole, with its floor set so the
           pole stands the record's distance over the rim to the truck; the wall is the
           mast's own timber (the Ubena's is stained with it). Without the field the
           platform below is drawn exactly as before. The find itself gives nothing above
           the deck: Lahn's Blatt 1 draws the mast as its broken stump. */
        ...
        if (bsk) {
          ...
          /* ── THE CROSS AT THE TRUCK ────────────────────────────────────────────────
             The Elbing seal draws a cross on a staff over the top, and both Bremen
             replicas carry one; the Arbeitskreis historischer Schiffbau reads it as the
             sign of a peaceful merchantman and says the reading is not provable. DATA
             (`top.cross`), drawn pale — the Ubena's is white — at the record's span on
             the record's staff, its foot on the truck. Tagged 'cross', not 'mast', so the
             mast's own span still lands truckM. */
          if (bsk.cross) {
            ...
            cg.add(st);
            /* the arms span fore and aft: from the broadside the cross is seen face on,
               which is how both Kiel photographs (from abeam and the quarter) show it */
    ...
    if (mk.rig === 'crabclaw') {
      ...
      /* ── THE SPARS ARE THE PLATE'S WHERE THE RECORD READS THEM (round 250) ─────────────
         mk.sail carries a read of THIS sail's spars off a photograph at a stated scale: the
         yard's and the boom's lengths from the tack, the boom's angle over the aft-horizontal,
         the yard's angle off the mast (the plate shows it lashed along the mast, its peak
         standing above the masthead), the cloth's run up the yard where the head stops short of
         the tip, the tack's station abaft the mast's foot, and the cloth's share of the straight
         triangle — the leech's hollow, counted in pixels. The area then FALLS OUT of the spars,
         and where it disagrees with the card's Sail area row the record says so in a field of
         its own (sailAreaContested). Without the read the class solves the spar from the
         recorded area, as before.
         ⚠ The tack was 0.22 L FORWARD of the mast, which put the fore sail's foot over the bows
         and the yard leaning 22° forward of the vertical: the 2009 broadside has both tacks at
    ...
    if (mk.shrouds) {
      ...
      /* ── THE SHROUDS SET UP WHERE THE RECORD SAYS (round 236). The class lands them at
         the mast's own station, spread over a channel there. The Bremen cog's set up ABAFT
         the mast, in the channel wale at the castle's forward corner (Lahn Blatt 1; Tanner
         2021 App. H: the stanchions and the two boards over the shrouds' lower ends were
         recovered), 2.2–3.9 m abaft the mast — which is why Tanner notes they "do not sit
         naturally" run from the masthead. With mast.shroudFixing.stationsU the lower ends
         are spread between the outermost stanchions, at the top of the boards that box the
         deadeyes, just outside them; without it the class channel stands where it did. */
...
/* ── THE BASKET TOP — the cog's masthead as the seals draw it (round 235) ─────────────
   A staved bucket on the pole, flaring from its floor to a battlemented rim: the Elbing
   seal of 1350 draws it with the same crenellation as the ship's two castles, and the
   Ubena von Bremen carries one. Sizes are the RECORD'S (`top.rimDiaM`, `baseDiaM`,
   `heightM`, `staves`): the wall is an open frustum of `staves` facets, its floor a ring
   from the mast to the wall, alternate staves standing a hand higher at the rim (the
   battlements), two hoops. The wall is drawn double-sided because a bucket is seen into
   from above and its far inner face would otherwise be sky. The group is keyed 'top' and
   named 'Basket top' so the dated-top count sees a masthead platform and the r235 audit
   rule can tell it from the class platform. */
function buildBasketTop(spec, mat, mats, mastR) {
  ...
  g.add(floor);
  /* the battlements: every other stave runs a hand past the rim, leaning out with the wall */
...
/* ── THE HULL'S HALF-BREADTH AT A HEIGHT (round 236): a fitting that stands against the
 * planking at a given height wants the skin there, not the rail's or the deck's. Bisects the
 * surface's v for the height, on a skin that rises monotonically with v. */
...
/* ── THE CHANNEL WALE (Rüstbalken), record-gated on mast.shroudFixing (round 236) ──────
 * The Bremen cog's shrouds do not set up at the mast: they run aft to a wale outside the
 * planking at the castle's forward corner, where three stanchions stand on the wale and rise
 * to the castle rail, two boards close the bay between them from the wale to the sheer over
 * the shrouds' lower ends, three crosspieces tie the stanchions above the sheer with vertical
 * boards between the lower two, and three inner stanchions stand inside the rail — Tanner
 * 2021 App. H, Figures 41 and 68; Lahn Blatt 1 in elevation. The record carries the plate's
 * stations forward of the heel (as u), the heights over the keel, and the counts; the class
 * defaults are the timbers' unread dimensions. Both sides are drawn; the record says the
 * port side is inferred. */
function buildChannelWale(S, H, F, group, timber) {
  ...
  for (const sgn of [-1, 1]) {
    ...
    /* the wale follows the planking, in short lengths, its inner face on the skin */
    ...
    /* the outer stanchions stand on the wale outside the skin and lean out with the flare */
    ...
    /* the two boards over the shrouds' lower ends, wale to sheer, on the stanchions' faces */
    ...
    /* the crosspieces, flush between the stanchions */
    for (const h of cross) {
      const yq = y0 + h;
      box(xAft - xFwd, 0.10, 0.10, xMid, yq, sgn * (zOn(yq) + moulded / 2), 'channel-wale-crosspiece', rot);
    }
    /* vertical boards between the lower two crosspieces, as the plate draws them */
    if (cross.length >= 2) {
    ...
    /* the inner stanchions, inside the rail, deck to castle deck, at the outer ones' stations */
...
function buildRaisedEnds(S, group) {
  ...
  const mk = (u0, u1, label, what) => {
    ...
    /* the deck: a planked strip lofted station by station at deck + dh, so it carries the
       hull's own sheer out to the end rather than lying flat across it. H.deck (r221): the
       raised end stands one deck-height over the WEATHER DECK, which is the sheer on every
       hull without deck.belowSheerM and the recorded depth under it otherwise; the wall's
       base above stays on the skin's own edge, since it is the shell carried up. */
    const dp = [], di = [];
    for (let k = 0; k <= N; k++) {
...
function buildFunnel(S, group) {
  ...
  for (let i = 0; i < n; i++) {
    const u = stations[i];
    let y = H.deck(u);                            // the casing stands on the DECK (r221)
    if (T && T.recorded)
...
function buildTurrets(S, group, mats) {
  ...
  stations.forEach((u, i) => {
    const base = H.deck(u);                        // the barbette stands on the deck (r221)
...
function buildSternAviation(S, group) {
  ...
  const deckY = H.deck(u);                         // the quarterdeck, not the skin's edge (r221)
...
function buildWingSail(S, group, mats) {
  ...
  const base = H.deck(u);                          // the bearing stands on the deck (r221)
  const x = (u - 0.5) * L;
...
/* ── THE WALLED CASTLES OF THE SOUTHERN TRADITION (r243) ─────────────────────────────
 * `castles: { fore: [fromU, toU, tiers], aft: [fromU, toU, tiers], deckHM?, railHM? }`
 * ⚠ Not `castle` (singular): that is the cog's northern OPEN PLATFORM on posts, drawn in
 * buildFittings from the wreck's own plan. A nau's castles are the other tradition —
 * WALLED houses carried up from the hull's side, the forecastle one deck over the bow, the
 * aftcastle rising in steps to the poop — and they are most of what makes a carrack read
 * as one: without them the Shipwright drew a flush-decked barque (r243, the carrack spun
 * from twelve bearings). Each tier is a wall lofted off the hull's own half-breadth and
 * sheer at its stations (the buildRaisedEnds idiom: a wall lofted off the real surface
 * cannot overhang), inset a step from the tier below, with a planked deck across its head
...
function buildTieredCastles(S, group, mats, hullMat) {
  ...
  const one = (label, spec, aft, what) => {
    ...
    for (const T of TIERS.filter(T => T.end === (aft ? 'aft' : 'fore'))) {
      ...
      path.push(path[0]);
      /* the wall: base sunk a tenth of a deck into what it stands on (no coplanar cap to
         fight), head at the tier's deck; uv in the hull shader's own frame */
      const tp = [], tuv = [], ti = [];
      let run = 0;
      for (let k = 0; k < path.length; k++) {
        const p = path[k];
      ...
      g.add(deck);
      /* the rail: posts resampled from the wall's own perimeter, one cap bar between them */
      const step = B * 0.20, Q = [];
      let acc = 0; Q.push(path[0]);
      for (let k = 1; k < path.length; k++) {
        acc += Math.hypot(path[k].x - path[k - 1].x, path[k].z - path[k - 1].z);
...
function buildPaddles(S, group, mats) {
  ...
  for (const sgn of [-1, 1]) {
    ...
    const h0 = Math.min(Math.max(H.deck(u) - axleY, boxRy * 0.12), boxRy * 0.55);   // the deck, r221
...
function buildShip(S, opts) {
  ...
  /* r235: and the TRUCK of that mast (rigTruckY) — the head of the mast meshes, which is what
     "deck to truck" measures to. rigTop is the model's highest point and stays the camera's
