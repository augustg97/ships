...
function buildGalleyWorks(S, group, mats) {
  ...
  if (AP) {
    ...
    /* a plank with eased edges — thousands of hours of rowing round a seat's arrises */
  ...
  if (S.gunDeck && AP) {
    ...
    const surfY = gdY + B * 0.007;                     // the plank's upper face
    for (const sgn of [-1, 1]) {
      /* the clamp — the fore-and-aft beam the deck edge sits on — and its stanchions,
      ...
      /* the screen: the gunners' and rowers' only cover, and the line the muzzles clear */
  ...
  if (S.bowGuns && !S.bowFortress) {
    ...
    /* a raised platform cannot float on its edge beams — posts to the bow deck,
       standing where the deck actually is under each corner (the head-timbers' law) */
    if (S.gunDeck) {
